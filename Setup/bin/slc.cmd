...
cls
title Digital License Activation [Mod By Aung Thu Myint]
set Unattended=
set _args=
set _elev=
set "_arg1=%~1"
...
if not exist "!_work!\" (
%ELine%
echo Folder does not exist in current directory.
...
for %%# in (%1 %2) do (if not exist "!_work!\%%#" (if defined _miss (set "_miss=!_miss! %%#") else (set "_miss=%%#")))
if defined _miss (
%ELine%
echo Following required file^(s^) is missing in folder. Aborting...
echo:
echo !_miss!
)
exit /b
